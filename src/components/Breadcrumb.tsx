      ...
      {items.map((item, i) => {
        ...
        return (
          <span key={i} className="flex items-center gap-1">
            ...
            {isLast || !item.href ? (
              <span className={isLast ? "font-semibold text-slate-800 dark:text-slate-100" : ""}>{item.label}</span>
            ) : (
              <Link href={item.href} className="text-indigo-600 hover:underline dark:text-indigo-400">
                {item.label}
              </Link>
            )}
          </span>
